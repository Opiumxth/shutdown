...
export function usePortalMatch(matchId: string) {
  ...
  // Ghost-damage guard: ignore any attack/result event older than the moment
  // this client mounted, so channel history from a previous session never
  // deals damage on load.
  ...
  const applyResult = useCallback(
    (turnId: string, role: TurnRole, success: boolean, timestamp: number) => {
      if (timestamp < matchStartTimeRef.current) return;
  ...
  const { status, presence, messages, me, send } = useChannel<MatchMessage>({
    ...
    onMessage: (msg) => {
      if (msg.type === "defense") {
        ...
        // Debug: confirm the passive attack actually arrives with isMinor.
        console.log("Ataque recibido en rival:", payload);
  ...
  useEffect(() => {
    ...
    for (const msg of messages) {
      ...
      if (msg.timestamp < matchStartTimeRef.current) continue;
  ...
  const attack = useCallback(
    async (
      ...
      console.log("Enviando ataque:", payload);
      void send({
        type: "attack",
        content: payload,
      });
